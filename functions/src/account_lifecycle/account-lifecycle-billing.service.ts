...
// -----------------------------------------------------------------------------
// A mudança de accountStatus persiste primeiro uma obrigação de interromper
// cobranças futuras. Este serviço converge essa obrigação com o domínio
// financeiro; indisponibilidade externa nunca desfaz o lifecycle já confirmado.
// -----------------------------------------------------------------------------
...
export async function reconcileAccountLifecycleBillingCancellation(input: {
  ...
  try {
    ...
    try {
      ...
      // O contrato já carrega needsProviderCancellation e o reconciliador de
      // billing continuará tentando; do ponto de vista do lifecycle, a
      // obrigação local já foi persistida com sucesso.
