...
export interface ExecuteVideoProcessingDispatchCommand {
  jobId: string;
...
export function normalizeVideoProcessingJob(
  data: unknown
...
async function claimQueuedJob(
  ...
  return db.runTransaction(async (transaction) => {
    ...
    const job = jobSnapshot.exists
      ? normalizeVideoProcessingJob(jobSnapshot.data())
      : null;

    if (!job || job.state !== 'QUEUED' || job.nextAttemptAt > now) {
      return null;
    }
...
export async function executeVideoProcessingDispatch(
  command: ExecuteVideoProcessingDispatchCommand
): Promise<void> {
  const jobId = cleanId(command.jobId);
  const processingVersion = cleanId(command.processingVersion);

  if (!jobId || !processingVersion) {
    throw new Error('Despacho de processamento inválido.');
  }
  ...
  const job = normalizeVideoProcessingJob(snapshot.data());

  if (!job) {
    await removeInvalidJob(jobRef);
    return;
  }

  if (job.processingVersion !== processingVersion) {
    return;
  }
  ...
  if (command.mode === 'SUBMIT') {
    if (job.state !== 'QUEUED' || job.nextAttemptAt > now) {
      return;
    }

    const claimedJob = await claimQueuedJob(jobRef, now);

    if (claimedJob) {
      await submitClaimedJob(jobRef, claimedJob);
    }
    return;
  }

  if (command.mode === 'RECOVER_SUBMISSION') {
    if (
  ...
  if (command.mode === 'RECONCILE') {
    ...
    await reconcileProcessingJob(jobRef, job);
    return;
  }

  if (job.state === 'CANCEL_REQUESTED') {
    await processCancellation(jobRef, job);
  }
}

export async function submitQueuedVideoProcessingRecoveryBatch(): Promise<void> {
  const now = Date.now();
  const submittingSnapshot = await db
    .collection(VIDEO_PROCESSING_JOBS_COLLECTION)
  ...
  await runBatch(
    submittingSnapshot.docs
      .map((jobDoc) => ({
        jobDoc,
        job: normalizeVideoProcessingJob(jobDoc.data()),
      }))
  ...
  await runBatch(tasks, 'submit-recovery');
}

export async function reconcileVideoProcessingRecoveryBatch(): Promise<void> {
  ...
  for (const jobDoc of snapshot.docs) {
    const job = normalizeVideoProcessingJob(jobDoc.data());
  ...
  await runBatch(tasks, 'reconcile-recovery');
}

export async function cleanupCancelledVideoProcessingRecoveryBatch(): Promise<void> {
  const snapshot = await db
    .collection(VIDEO_PROCESSING_JOBS_COLLECTION)
  ...
  await runBatch(tasks, 'cancel-recovery');
}
