// functions/src/media/application/video-edit-recipe.ts
// -----------------------------------------------------------------------------
// VIDEO EDIT RECIPE
// -----------------------------------------------------------------------------
// A edição é declarativa e não destrutiva. O navegador envia somente a receita;
// o arquivo original permanece privado e o mesmo job de transcodificação aplica
// corte temporal, enquadramento e remoção de áudio.
// -----------------------------------------------------------------------------
...
function normalizeSourceDuration(value: unknown): number | null {
  ...
  if (
    normalized === null ||
    normalized < MIN_EDITED_DURATION_MS ||
...
export function normalizeVideoEditRecipe(
  ...
  if (sourceDurationMs !== null) {
    ...
    if (effectiveEnd - requestedStart < MIN_EDITED_DURATION_MS) {
      throw new VideoEditRecipeValidationError(
