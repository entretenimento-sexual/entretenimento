...
export interface VideoEditRecipe {
  ...
  readonly rotationDegrees: VideoRotationDegrees;
...
function normalizeRotationDegrees(value: unknown): VideoRotationDegrees {
  if (value === null || value === undefined || value === '') {
    return 0;
  }
  ...
  throw new VideoEditRecipeValidationError(
    'A rotação do vídeo precisa ser 0°, 90°, 180° ou 270°.'
  );
}
...
function normalizeSourceDuration(value: unknown): number | null {
  ...
  if (
    ...
    normalized > MAX_SOURCE_DURATION_MS
  ) {
    throw new VideoEditRecipeValidationError(
...
export function normalizeVideoEditRecipe(
  ...
  const rotationDegrees = normalizeRotationDegrees(raw['rotationDegrees']);
  ...
  if (
    ...
    throw new VideoEditRecipeValidationError(
      'As dimensões do vídeo são necessárias para aplicar a rotação.'
    );
  }
...
function rotatedDimensions(recipe: VideoEditRecipe): {
  width: number | null;
  height: number | null;
} {
  ...
  const quarterTurn =
    recipe.rotationDegrees === 90 || recipe.rotationDegrees === 270;
...
export function withoutVideoRotation(
  recipe: VideoEditRecipe
): VideoEditRecipe {
  ...
  const dimensions = rotatedDimensions(recipe);
