...
if (fs.existsSync(helperPath)) {
  ...
  for (const optionalHelper of [
    ...
    const helperEnd = helperStart >= 0 ? source.indexOf('\n    }', helperStart) : -1;
    const helperSource = helperStart >= 0 && helperEnd >= 0
    ...
    if (
      ...
      violations.push(
        `firestore-rules/_helpers.rules (${optionalHelper} deve ser null-safe para campos opcionais ausentes)`
      );
    }
  }
}
...
if (fs.existsSync(requestsRepoPath)) {
  ...
  if (!/getPendingRequestsCallable\(\)[\s\S]*?this\.inCtxSync\(\(\) =>[\s\S]*?httpsCallable/.test(source)) {
    violations.push(
      'src/app/core/services/interactions/friendship/repo/requests.repo.ts (httpsCallable de pending requests deve nascer dentro do Angular injection context)'
    );
  }
}
