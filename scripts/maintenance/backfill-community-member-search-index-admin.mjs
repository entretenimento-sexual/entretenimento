// scripts/maintenance/backfill-community-member-search-index-admin.mjs
// -----------------------------------------------------------------------------
// BACKFILL DO ÍNDICE DE BUSCA PÚBLICA DE INTEGRANTES
// -----------------------------------------------------------------------------
// Dry-run por padrão. A projeção usa somente membership + public_profiles.
// Não altera membership, perfil, roles, ownership, elegibilidade ou billing.
//
// PowerShell:
// $env:FIREBASE_PROJECT_ID='entretenimento-sexual'
// $env:COMMUNITY_MEMBER_SEARCH_INDEX_DRY_RUN='true'
// npm run maintenance:community-member-search-index
//
// Escrita real, somente após índices READY e triggers publicados:
// $env:COMMUNITY_MEMBER_SEARCH_INDEX_DRY_RUN='false'
// $env:COMMUNITY_MEMBER_SEARCH_INDEX_CONFIRM='true'
// npm run maintenance:community-member-search-index
// -----------------------------------------------------------------------------
...
const dryRun =
  String(process.env.COMMUNITY_MEMBER_SEARCH_INDEX_DRY_RUN || 'true')
    .trim()
    .toLowerCase() !== 'false';
const confirmed =
  String(process.env.COMMUNITY_MEMBER_SEARCH_INDEX_CONFIRM || 'false')
    .trim()
    .toLowerCase() === 'true';
const pageSize = Math.max(
  ...
  Math.min(
    300,
    Number.parseInt(
      String(process.env.COMMUNITY_MEMBER_SEARCH_INDEX_PAGE_SIZE || '200'),
      10
    ) || 200
  )
);
const maxCommunities = Math.max(
  ...
  Math.min(
    100_000,
    Number.parseInt(
      String(process.env.COMMUNITY_MEMBER_SEARCH_INDEX_MAX_COMMUNITIES || '10000'),
      10
    ) || 10_000
  )
);
const maxMemberships = Math.max(
  ...
  Math.min(
    5_000_000,
    Number.parseInt(
      String(process.env.COMMUNITY_MEMBER_SEARCH_INDEX_MAX_MEMBERSHIPS || '1000000'),
      10
    ) || 1_000_000
  )
);
...
function countSearchPrefixCompositeIndexes() {
  ...
  return indexes.filter((index) =>
    index?.collectionGroup === 'community_member_search_index'
...
const searchPrefixCompositeIndexCount = countSearchPrefixCompositeIndexes();
...
async function loadPolicy() {
  try {
    const module = await import(
      '../../functions/lib/community/community-member-search-index.policy.js'
    );

    if (
      typeof module.buildCommunityMemberSearchIndexProjection !== 'function'
      || typeof module.communityMemberSearchProjectionId !== 'function'
    ) {
      throw new Error('Policy canônica do índice de busca não encontrada.');
    }
...
function isSearchableCommunity(data) {
...
async function processCommunity(db, policy, communityDocument, counters) {
  ...
  while (counters.scannedMemberships < maxMemberships) {
    ...
    const profileSnapshots = await readPublicProfiles(db, snapshot.docs);
    const writes = [];

    snapshot.docs.forEach((document, index) => {
      const projection = policy.buildCommunityMemberSearchIndexProjection({
        communityId: communityDocument.id,
        memberId: document.id,
        rawMembership: document.data(),
        rawPublicProfile: profileSnapshots[index]?.exists
      ...
      const ref = db
        ...
        .doc(
          policy.communityMemberSearchProjectionId(
      ...
      if (!projection) {
        counters.skipped += 1;
        counters.deleteCandidates += 1;
        writes.push({ ref, projection: null });
        return;
      }

      counters.projected += 1;
      const prefixCount = Array.isArray(projection.searchPrefixes)
        ? projection.searchPrefixes.length
        : 0;
      counters.searchPrefixValues += prefixCount;
      ...
      counters.estimatedSearchPrefixCompositeEntries +=
        prefixCount * searchPrefixCompositeIndexCount;
      writes.push({ ref, projection });
    });
    ...
    console.log('[community-member-search-index] progresso', {
      communityId: communityDocument.id,
...
async function main() {
  if (!dryRun && !confirmed) {
    throw new Error(
      'Execução real bloqueada. Defina COMMUNITY_MEMBER_SEARCH_INDEX_CONFIRM=true.'
    );
  }
  ...
  while (
    ...
    for (const communityDocument of snapshot.docs) {
      ...
      if (!isSearchableCommunity(communityDocument.data())) continue;

      counters.searchableCommunities += 1;
      ...
      try {
        ...
        counters.failures += 1;
        console.error('[community-member-search-index] falha', {
          communityId: communityDocument.id,
  ...
  console.log('[community-member-search-index] resumo', {
    projectId,
    dryRun,
    pageSize,
    maxCommunities,
    ...
    averageSearchPrefixesPerProjection:
      counters.projected > 0
        ? Number((counters.searchPrefixValues / counters.projected).toFixed(2))
        : 0,
    ...counters,
    complete,
    truncated:
...
main().catch((error) => {
  console.error('[community-member-search-index] erro fatal', error);
  process.exitCode = 1;
});
